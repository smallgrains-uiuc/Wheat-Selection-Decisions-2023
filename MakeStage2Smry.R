...
###########Analysis is to select among stage 2 lines
...
#subset the the Prelim (stage 2 lines) and checks
cks<- c("07-19334", "12-26004", "16-8048", "Kaskaskia", 'Pio 25R74', 'AgriMAXX503','25R76')
lines2023<- read.csv('YT blues 2023.csv', as.is=TRUE, row.names=1)$germplasmName
Pr_lines<- unique(lines2023[grep("IL2021-", lines2023)])
allsub4<- allsub3[which(allsub3$germplasmName %in% c(cks, as.character(Pr_lines))),]
allsub4[which(allsub4$value.x==4.3),c('value.x','value.y')]<- c(NA, 0)

#trials to include
ixel<- which(allsub4$study %in% c('YT_Urb_23', 'YT_Urb_22', 'YT_Neo_23', 'YT_Neo_22', 'YT_Addie_23','YT_Stp_23'))
allsub4<- allsub4[ixel,]
...
#DON
don<- read.csv("GEBV_onlyDON.csv")
don$trait<- 'DON'
don$study<- NA
donBLUP<- don[,colnames(matBLUP)]
donBLUP<- donBLUP[which(donBLUP$germplasmName %in% c(cks, as.character(Pr_lines))),]

#combine all the BLUPS
all<- rbind(yldBLUPavg, twBLUPavg, htBLUP,hdBLUP, matBLUP, lodgBLUP, donBLUP,yldBLUP, twBLUP)
...
#write.csv(all_wide$germplasmName, file='stage2lines2023_forselection.csv')
...
all_wide<- all_wide[,c(1,14,7,3,6,4,2,5,15:20,8:13,21)]
write.csv(all_wide, file='SelectFromStage2_2023.csv')
